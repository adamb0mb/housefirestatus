// Core live-status computation — the same logic used by /api/status and by the
// cron job that re-checks every subscribed address (see subscriptions.js). Kept
// in its own module (rather than living in index.js, which imports from
// subscriptions.js) purely to avoid a circular import between the two.
...
// Core status computation, shared by the /api/status HTTP handler and the cron
// job that re-checks every subscribed address. Returns the plain payload object
// (no Response wrapping, no caching) so the cron job can diff it directly.
export async function computeStatus(lat, lng, ctx) {
  const pointParams = {
    geometry: `${lng},${lat}`,
